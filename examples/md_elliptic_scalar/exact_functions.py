...
def u_exact(x, y, z, m_c1, m_c2, m_kappa, m_delta):
    u_1_val = np.array(
        [
            [
                -(m_c1 * (0.5 - y) + 1.0) * grad_p_f_exact(x, y, z)[0],
    ...
    u_2_val = np.array(
        [
            [
                -(m_c2 * (y - 0.5) + 1.0) * grad_p_f_exact(x, y, z)[0],
...
def f_c0_rhs(x, y, z, m_c1, m_c2, m_kappa, m_delta):
    f_1 = np.array(
        [4 * (np.pi**2) * (m_c1 * (0.5 - y) + 1.0) * p_f_exact(x, y, z)]
    )
    f_2 = np.array(
        [4 * (np.pi**2) * (m_c2 * (y - 0.5) + 1.0) * p_f_exact(x, y, z)]
    )
