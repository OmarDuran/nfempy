...
def u_f_exact(x, y, z, m_c, m_kappa, m_delta):
    return np.array([- f_kappa(x, y, z, m_kappa) * f_delta(x, y, z, m_delta) * grad_p_f_exact(x, y, z)])
...
def u_exact(x, y, z, m_c, m_kappa, m_delta):
    return np.array(
        [
            [
                -m_c * p_f_exact(x, y, z),
                - (m_c * x + f_delta(x, y, z, m_delta)) * grad_p_f_exact(x, y, z),
            ]
        ]
    )

def f_c0_rhs(x, y, z, m_c, m_kappa, m_delta):
    return np.array([[4 * (np.pi**2) * (m_c * x + f_delta(x, y, z, m_delta)) * p_f_exact(x, y, z)]])

def f_c1_rhs(x, y, z, m_c, m_kappa, m_delta):
    return np.array([[4 * (np.pi**2) * f_kappa(x, y, z, m_kappa) *  f_delta(x, y, z, m_delta)  * np.sin(2 * np.pi * y)]])
