...
def f_grad_d_phi(x, y, z, m_par, m_mu, dim):
    if dim == 1:
        scalar_part = f_porosity(x, y, z, m_par, dim) / (
            m_mu * f_d_phi(x, y, z, m_par, m_mu, dim)
        )
        vector_part = f_grad_porosity(x, y, z, m_par, dim)
        return scalar_part * vector_part
    elif dim == 2:
        scalar_part = np.where(
            np.logical_or(x <= -3 / 4, y <= -3 / 4),
            np.zeros_like(x),
            f_porosity(x, y, z, m_par, dim)
            / (m_mu * f_d_phi(x, y, z, m_par, m_mu, dim)),
        )
        vector_part = f_grad_porosity(x, y, z, m_par, dim)
        return scalar_part * vector_part
...
def q_exact(x, y, z, m_par, dim):
    if dim == 1:
        ...
        return np.where(
            ...
            np.array(
                [
                    np.sqrt(((0.75 + x) ** gamma) * ((0.75 + y) ** (2 * gamma)))
                    * np.cos(6 * x * (y**2))
                ]
            ),
        )
...
def v_exact(x, y, z, m_par, m_mu, dim):
    if dim == 1:
        return np.where(
            x < 0.0,
            np.array([x * 0.0]),
            u_exact(x, y, z, m_par, dim) / f_d_phi(x, y, z, m_par, m_mu, dim),
        )
    elif dim == 2:
        gamma = m_par
        return np.where(
...
def f_rhs(x, y, z, m_par, dim):
    if dim == 1:
        ...
        return np.where(
            ...
            np.array([[x * 0.0]]),
            np.array(
                [[(np.abs(x) ** beta) * np.sqrt(f_porosity(x, y, z, m_par, dim))]]
            ),
        )
    elif dim == 2:
        gamma = m_par
        return np.where(
            ...
            np.array(
                [
                    [
                        np.sqrt(f_porosity(x, y, z, m_par, dim))
                        * (
                            (
                                1
                                + 36
