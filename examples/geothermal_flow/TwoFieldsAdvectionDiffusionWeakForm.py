...
class TwoFieldsDiffusionWeakForm(WeakForm):
    def evaluate_form(self, element_index, alpha_n_p_1, alpha_n, t):
        ...
        with ad.AutoDiff(alpha_n_p_1) as alpha_n_p_1:
            ...
            # for l, idx in enumerate(sub_entities):
            #     for i, omega in enumerate(c1_weights):
            #
            #         # Trace of functions at integration point i
            #         xv = trace_x[l]
            #         n = normals[l]
            #         tr_dof = trace_q_dof[l]
            #         dq_h = trace_q_space[l][i].reshape(1,tr_dof.shape[0])
            #         du_h = trace_u_space[l][i]
            #
            #
            #         # Dof per field
            #         alpha_q_n_p_1 = alpha_n_p_1[:, tr_dof]
            #         alpha_u_n_p_1 = alpha_n_p_1[:, idx_dof["u"]]
            #
            #         # FEM approximation
            #         q_h_n_p_1 = alpha_q_n_p_1 @ dq_h
            ...
            #
            #         beta_upwind = 0.0
            #         # if q_h_n_p_1.val[0,0] > 0.0 or np.isclose(q_h_n_p_1.val[0,0],0.0):
            #         #     beta_upwind = 1.0
            #         v_n = f_velocity(xv[:,0], xv[:,1], xv[:,2]) @ n
            #         if v_n > 0.0 or np.isclose(v_n,0.0):
            #             beta_upwind = 1.0
            #
            #         equ_1_integrand = (beta_upwind * (u_h_n_p_1 * v_n) @ du_h.T)
            #
            #         multiphysic_integrand = np.zeros((1, n_dof))
    ...
    def evaluate_form(self, element_index, alpha, t):
        ...
        for c in range(q_components):
            ...
            for i, omega in enumerate(weights):
                ...
                v_n = f_velocity(x[:, 0], x[:, 1], x[:, 2]) @ n
                beta_scale = beta_v - bc_sign * v_n
                # beta_scale = beta_v
                bc_residual_equ = (1.0 / beta_scale) * (
                    q_h_n + beta_v * c_v - gamma_v * c_v
                )
...
class TwoFieldsAdvectionWeakForm(WeakForm):
    def evaluate_form(self, cell_c1, element_pair_index, alpha_pair):
        ...
        gmesh = q_space.mesh_topology.mesh
        element_c1_data = ElementData(cell_c1, q_space.mesh_topology.mesh)
        points, weights = self.space.bc_quadrature
        ...
        idx_dof = {
            "q_p": slice(0, n_q_dof),
            "u_p": slice(n_q_dof, n_q_dof + n_u_dof),
            "q_n": slice(n_q_dof + n_u_dof, 2 * n_q_dof + n_u_dof),
            "u_n": slice(2 * n_q_dof + n_u_dof, 2 * n_q_dof + 2 * n_u_dof),
        }

        n_dof = 2 * n_q_dof + 2 * n_u_dof
        js = (n_dof, n_dof)
        rs = n_dof
        j_el = np.zeros(js)
        r_el = np.zeros(rs)
        ...
        with ad.AutoDiff(alpha) as alpha:
            ...
            for i, omega in enumerate(weights):
                ...
                equ_1_integrand = +1.0 * (u_h_upwind * v_n) @ du_h_p.T
                equ_2_integrand = -1.0 * (u_h_upwind * v_n) @ du_h_n.T
    ...
    def evaluate_form(self, cell_c1, element_index, alpha):
        ...
        with ad.AutoDiff(alpha) as alpha:
            ...
            for i, omega in enumerate(weights):
                ...
                equ_1_integrand = (u_h_upwind * v_n) @ du_h.T
