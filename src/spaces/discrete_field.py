...
class DiscreteField:
    ...
    def build_bc_dof_map(self, only_on_physical_tags=True):
        st = time.time()
        if only_on_physical_tags:
        ...
        et = time.time()
        elapsed_time = et - st
        print("DiscreteField:: Boundary DoFMap construction time:", elapsed_time, "seconds")

    def build_elements(self, parallel_run_q = False):
        st = time.time()
    ...
    def build_bc_elements(self, physical_tags):
        st = time.time()

        self.bc_element_ids = self.bc_mesh_topology.entities_by_dimension(self.dimension-1)
        if self.physical_tag_filter:
            mesh = self.bc_mesh_topology.mesh
            self.bc_element_ids = [
                id for id in self.bc_element_ids if mesh.cells[id].material_id in physical_tags
            ]
        ...
        self.id_to_bc_element = dict(zip(self.bc_element_ids, range(len(self.bc_element_ids))))
        et = time.time()
        elapsed_time = et - st
        n_d_cells = len(self.bc_elements)
        print("DiscreteField:: Number of processed bc elements:", n_d_cells)
        print("DiscreteField:: Boundary Elements construction time:", elapsed_time, "seconds")
