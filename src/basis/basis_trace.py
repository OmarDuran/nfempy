...
def trace_discrete_space(sub_space: DiscreteSpace, points, c1_data: ElementData, return_all_dof_q, neigh_idx = 0):
...
def trace_product_space(fields, space:ProductSpace, points, c1_data: ElementData, return_all_dof_q, neigh_idx=0):
    traces_map = {}
    sub_spaces = {field: space.discrete_spaces[field] for field in fields}
    for item in sub_spaces.items():
        field, sub_space = item
        tr_phi = trace_discrete_space(sub_space, points, c1_data, return_all_dof_q, neigh_idx)
