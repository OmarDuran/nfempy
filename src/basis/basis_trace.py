...
def _trace(points, c1_data: ElementData, c0_element: FiniteElement):
    c1_cell = c1_data.cell
    c0_cell = c0_element.data.cell

    mapped_points = transform_lower_to_higher(points, c1_data, c0_element.data)
    _, jac_c0, det_jac_c0, inv_jac_c0 = c0_element.evaluate_mapping(
        mapped_points
    )
...
def trace_discrete_space(sub_space: DiscreteSpace, points, c1_data: ElementData, neigh_idx = 0):
    ...
    tr_phi = _trace(points, c1_data, c0_element)
    return tr_phi

def trace_product_space(fields, space:ProductSpace, points, c1_data: ElementData, neigh_idx=0):
    traces_map = {}
    sub_spaces = {field: space.discrete_spaces[field] for field in fields}
    for item in sub_spaces.items():
        field, sub_space = item
        tr_phi = trace_discrete_space(sub_space, points, c1_data, neigh_idx)
