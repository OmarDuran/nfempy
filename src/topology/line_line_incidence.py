...
def line_line_intersection(a: np.array, b: np.array, c: np.array, d: np.array, eps: float = l_incidence_tol) -> float:
...
def lines_line_intersection(lines: np.array, a: np.array, b: np.array, eps: float = l_incidence_tol) -> float:
...
def lines_lines_intersection(lines_tools: np.array, lines_objects, eps: float = l_incidence_tol) -> float:
    # compute intersections

    result = lines_objects.shape[0] * [None]
    for i, line_obj in enumerate(lines_objects):
        line_equality = [np.all(np.isclose(line_tool, line_obj)) for line_tool in lines_tools]
        tuple_idx = np.where(line_equality)
        if len(tuple_idx) > 0:
            idx = tuple_idx[0]
            filtered_lines_tools = np.delete(lines_tools, idx, axis=0)
            result[i] = lines_line_intersection(filtered_lines_tools, line_obj[0], line_obj[1], eps)
        else:
            result[i] = lines_line_intersection(lines_tools, line_obj[0], line_obj[1], eps)
    return result
...
def line_line_plot(a: np.array, b: np.array, c: np.array, d: np.array):
    ...
    if len(out) > 0:
        ...
        if out.shape == (1,3):
            ...
            ax.plot3D([c[0], d[0]], [c[1], d[1]], [c[2], d[2]], "o-", label='Intersection segment')
