...
def domain_intersection(
    ...
    c0_shapes_object = [
        shape
        for dim_shape in c0_object.shapes
        for shape in dim_shape
        if max_dim - shape.dimension == 0
    ]
    c1_shapes_tool = [
        shape
        for dim_shape in c1_tool.shapes
        for shape in dim_shape
        if max_dim - shape.dimension == 1
    ]
...
def domain_difference(
    domain_minuend: Domain,
    domain_subtrahend: Domain,
    tag: int = tag_info.min,
    eps: float = p_incidence_tol,
):
    ...
    c0_shapes_object = [
        shape
        for dim_shape in domain_minuend.shapes
        for shape in dim_shape
        if max_dim - shape.dimension == 0
    ]
    c1_shapes_tool = [
        shape
        for dim_shape in domain_subtrahend.shapes
        for shape in dim_shape
        if max_dim - shape.dimension == 1
    ]
    ...
    for edge, shape_edges, shape_vertices in output:
        for shape in shape_edges:
            ...
            shapes_by_co_dimension[co_dimension][shape.index(max_dimension=max_dim)] = (
                shape
            )
...
def domain_union(
    ...
    for shape_by_dimension in domain_c0.shapes:
        for shape in shape_by_dimension:
            co_dimension = max_dim - shape.dimension
            all_shapes[co_dimension][shape.index(max_dimension=max_dim)] = copy.copy(
                shape
            )
    ...
    for v_tool in domain_c1.shapes[0]:
        ...
        chunk = [
            vertex.index(max_dimension=max_dim)
            for vertex in domain_c0.shapes[0]
            if vertex_with_same_geometry_q(vertex, v_tool)
        ]
    ...
    for shape_idx, c0_indexed_shape in enumerate(c0_indexed_shapes):
        ...
        shape_successors = nx.dfs_successors(
            md_G_partial, source=c0_indexed_shape, depth_limit=max_dim
        )
