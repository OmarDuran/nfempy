...
def __projection_directions(points):
    ...
    pos = np.argmax(np.abs([np.dot(normal,e1), np.dot(normal,e2), np.dot(normal,e3)]))
...
def convex_q(points: np.array):
    ...
    eged_directions = np.array([__edge_direction(points[i],points[(i + 1) % n_points], points[(i + 2) % n_points], normal) for i in range(n_points)])
