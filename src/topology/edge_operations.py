...
def edge_with_same_geometry_q(
    edge_a: Edge, edge_b: Edge, eps: float = collapse_tol
):
...
def collapse_edge(
    e_tool: Edge, e_object: Edge, eps: float = collapse_tol
) -> Vertex:
...
def edge_edge_boundary_intersection(
    e_tool: Edge, e_object: Edge, eps: float = p_incidence_tol
) -> Union[None, Vertex]:
    ...
    bc_out_tv0 = vertex_edge_boundary_intersection(tv0, e_object)
    bc_out_tv1 = vertex_edge_boundary_intersection(tv1, e_object)
    ...
    # intersection inside
    out_tv0 = vertex_edge_intersection(tv0, e_object)
    out_tv1 = vertex_edge_intersection(tv1, e_object)
...
def edge_edge_intersection(
    e_tool: Edge, e_object: Edge, eps: float = l_incidence_tol
) -> Union[None, Vertex]:
    bc_out = edge_edge_boundary_intersection(e_tool, e_object, eps)
